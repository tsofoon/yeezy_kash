...
def load_data(suffix = '_all_shoe_down10_up10'):
    # load look up tables for label encoding of each of the catagorical features
...
def predict(q_product_info, q_shoe_size, activity_df):
    # perform prediction based on product info, quried shoe size and past transaction record of shoe
    # return predictions from price drop and price drop models

    # parse product detail and convert to label encoding
    ...
    # convert activity_df to the right format for prediction
    ...
    features = ['model', 'color1', 'color2', 'product_name', 'shoe_size',
       ...
       'd_last_createdAt_day_same_size', 'd_last_createdAt_day_any_size']

    up_model, down_model = get_up_model(), get_down_model() # load lightGBM models
    # make predictions
    ...
    # return only the prediction of the last transaction
